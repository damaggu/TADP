_base_ = [
    ...
    '_base_/datasets/cityscapes_to_darkzurich_512x512.py', ### for cross domain
...
optimizer = dict(type='AdamW', lr=0.00008, weight_decay=0.001, #0.005
                 paramwise_cfg=dict(custom_keys={'unet': dict(lr_mult=0.1),
                                                 'encoder_vq': dict(lr_mult=0.0),
                                                 'text_encoder': dict(lr_mult=0.0),
                                                 'norm': dict(decay_mult=0.)}))
data = dict(samples_per_gpu=1, workers_per_gpu=8)
n_gpus = 1
...
name = 'citscapes_to_darkzurich_512x512'
exp = 'basic'
name_dataset = 'cityscapes_to_darkzurich_512x512'
